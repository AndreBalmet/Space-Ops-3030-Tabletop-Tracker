...
// Shared scripts for node builder previews
// Top-level event listeners are null-guarded so they don't crash in partial node previews
{

// Firebase config — may already be defined by the builder's injection script or mock
if (typeof firebaseConfig === 'undefined') {
    var firebaseConfig = {
        ...
        let gameData = null; // Will be set after gameDataHardcoded is declared
        ...
        // Set gameData from hardcoded (moved here to avoid temporal dead zone)
        gameData = gameDataHardcoded;
        ...
        // ========================================
        // NODE BUILDER: Mock session join helper
        // Bypasses DOM dependencies so Game Area can render in isolated preview
        // ========================================
        window.mockJoinSession = function(sessionName, playerName) {
            currentSession = sessionName;
            currentPlayer = playerName.replace(/[.#$[\]\s]/g, '_');
            currentPlayerDisplay = playerName;
            console.log(`[MockJoin] Joined session "${sessionName}" as "${playerName}"`);

            // Update DOM elements if they exist
            const csEl = document.getElementById('currentSession');
            const cpEl = document.getElementById('currentPlayer');
            if (csEl) csEl.textContent = sessionName;
            if (cpEl) cpEl.textContent = playerName;

            // Show game area
            const ga = document.getElementById('gameArea');
            if (ga) ga.style.display = 'block';

            // Load campaign info if function exists
            try { loadCampaignInfo(); } catch(e) {}
            try { loadMissionObjectives(); } catch(e) {}

            // Set up teams listener — this renders the characters
            database.ref('sessions/' + sessionName + '/teams').on('value', (snapshot) => {
                renderTeams(snapshot.val() || {});
            });

            // Load combat log
            try { loadCombatLog(); } catch(e) {}

            // Load initiative
            try {
                database.ref('sessions/' + sessionName + '/initiative').on('value', (snapshot) => {
                    if (snapshot.val()) renderInitiative(snapshot.val());
                });
            } catch(e) {}
        };

        // Auto-join mock session if MOCK_DATA is available and gameArea exists in this node
        // Must wait for DOMContentLoaded since shared-scripts.js loads in <head> before body is parsed
        document.addEventListener('DOMContentLoaded', function() {
            if (typeof MOCK_DATA !== 'undefined' && document.getElementById('gameArea')) {
                console.log('[MockJoin] Auto-joining SpaceOps_Dec18 as Andre...');
                window.mockJoinSession('SpaceOps_Dec18', 'Andre');
            }
        });
        ...
        const _charForm = document.getElementById('characterForm');
        if (_charForm) _charForm.addEventListener('submit', function(e) {
            e.preventDefault();
...
`;
        ...
        window.addEventListener('load', function() {
            ...
            const savedPlayerName = localStorage.getItem('lastPlayerName');
            const _playerNameInput = document.getElementById('playerName');
            if (savedPlayerName && _playerNameInput) {
                _playerNameInput.value = savedPlayerName;
            }
            ...
            } catch(e) { console.warn('[SharedScripts] Load init:', e.message); }
        });
